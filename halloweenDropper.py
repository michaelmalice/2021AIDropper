...
def build_argparser():
    ...
    args.add_argument('-m', '--model', help='Required. Path to an .xml file with a trained model.',
                      required=False, type=Path, default='/home/michaellarsen/models/intel/person-detection-retail-0013/FP16/person-detection-retail-0013.xml')
    args.add_argument('-at', '--architecture_type', help='Required. Specify model\' architecture type.',
                      type=str, required=False, default='ssd', choices=('ssd', 'yolo', 'yolov4', 'faceboxes', 'centernet', 'ctpn',
    ...
    common_model_args.add_argument('-t', '--prob_threshold', default=0.7, type=float,
...
def draw_detections(frame, detections, palette, labels, threshold, output_transform):
    ...
    cv2.rectangle(frame, (0, 0), (200, 300), (0, 0, 255), 2)
    spookyxMin = 0
    spookyxMax = 0
    spookyyMin = 0
    spookyyMax = 0
    for detection in detections:
        if detection.score > .7: #threshold:
            personDetected = True
    ...
    if mqttCounter == 0:
        if personDetected:
           print(str(spookyxMin) + '  ' + str(spookyxMax) + '  ' + str(spookyyMin) + '  ' + str(spookyyMax))
        if personDetected and spookyxMin >= 0 and spookyxMax <= 200 and spookyyMin >= 0 and spookyyMax <= 300:
            publish.single('home-assistant/hillsboro-porch/person', 'true', hostname=broker)
            
        else:
            publish.single('home-assistant/hillsboro-porch/person', 'false', hostname=broker)
...
def main():
    ...
    cap = open_images_capture('rtsp://10.0.0.43:8554/hillsboro-porch', args.loop)  #args.input, args.loop)
